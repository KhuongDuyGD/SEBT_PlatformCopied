...
/**
 * Component Filter nâng cao cho pin điện
 * Hỗ trợ đầy đủ các tiêu chí filter theo database schema mới
 */
const BatteryAdvancedFilter = ({ onFilter, loading = false, initialValues = {} }) => {
  ...
  const loadFilterData = async () => {
    setLoadingData(true);
    try {
      const [provinces, brands, names, years, compatibleVehicles] = await Promise.all([
        listingsApi.getProvinces(),
        listingsApi.getBatteryBrands(),
        listingsApi.getBatteryNames(),
        listingsApi.getBatteryYears(),
        listingsApi.getCompatibleVehicles()
      ]);
  ...
  const handleSubmit = (values) => {
    ...
    console.log('[BATTERY_FILTER_SUBMIT]', cleanedValues);
  ...
  return (
    <Card 
      title={
        <Space>
          <FilterOutlined />
          <Title level={4} style={{ margin: 0 }}>Lọc pin điện nâng cao</Title>
        </Space>
      }
      size="small"
      ...
      >
        ...
        <Collapse ghost>
          <Panel header="Bộ lọc nhanh" key="presets">
            <Space wrap>
              <Button 
                size="small" 
                onClick={() => applyPreset({ conditionStatus: 'EXCELLENT', minHealthPercentage: 90 })}
              >
                Pin xuất sắc + sức khỏe tốt
              </Button>
              <Button 
                size="small"
                onClick={() => applyPreset({ minYear: 2023, minBatteryCapacity: 60 })}
              >
                Pin mới + dung lượng cao
              </Button>
              <Button 
                size="small"
                onClick={() => applyPreset({ minHealthPercentage: 80, conditionStatus: 'GOOD' })}
              >
                Sức khỏe tốt + tình trạng tốt
              </Button>
              <Button 
                size="small"
                onClick={() => applyPreset({ minBatteryCapacity: 70 })}
              >
                Dung lượng lớn (70kWh+)
              </Button>
            </Space>
          </Panel>
        </Collapse>
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} md={8}>
            <Form.Item label="Thương hiệu pin" name="brand">
              <AutoComplete
          ...
          {/* Tên pin - Autocomplete */}
          <Col xs={24} sm={12} md={8}>
            <Form.Item label="Tên pin" name="name">
              <AutoComplete
                placeholder="Nhập tên pin"
                options={filterData.names.map(name => ({ value: name, label: name }))}
                filterOption={(inputValue, option) =>
                  option.value.toLowerCase().includes(inputValue.toLowerCase())
                }
                allowClear
          ...
          {/* Tình trạng pin */}
          <Col xs={24} sm={12} md={8}>
            <Form.Item label="Tình trạng pin" name="conditionStatus">
          ...
          {/* Xe tương thích */}
          <Col xs={24} sm={12} md={8}>
            <Form.Item label="Xe tương thích" name="compatibility">
              <AutoComplete
                placeholder="Nhập loại xe tương thích"
                options={filterData.compatibleVehicles.map(vehicle => ({ value: vehicle, label: vehicle }))}
                filterOption={(inputValue, option) =>
                  option.value.toLowerCase().includes(inputValue.toLowerCase())
                }
                allowClear
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} md={6}>
            <Form.Item label="Dung lượng từ (kWh)" name="minBatteryCapacity">
              <InputNumber 
                placeholder="30"
                min={20}
                max={150}
          ...
          {/* Dung lượng pin - Max */}
          <Col xs={24} sm={12} md={6}>
            <Form.Item label="Dung lượng đến (kWh)" name="maxBatteryCapacity">
              <InputNumber 
                placeholder="100"
                min={20}
                max={150}
          ...
          {/* Sức khỏe pin - Min */}
          <Col xs={24} sm={12} md={6}>
            <Form.Item label="Sức khỏe từ (%)" name="minHealthPercentage">
              <InputNumber 
                placeholder="60"
                min={50}
                max={100}
          ...
          {/* Sức khỏe pin - Max */}
          <Col xs={24} sm={12} md={6}>
            <Form.Item label="Sức khỏe đến (%)" name="maxHealthPercentage">
              <InputNumber 
                placeholder="100"
                min={50}
                max={100}
...
export default BatteryAdvancedFilter;
